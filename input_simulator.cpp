...
void InputSimulator::SendAnimationCancelMove() {
	Sleep(1);
	HWND hwnd = FindWindow(NULL, _T("Diablo III"));
	RECT rc;
	GetWindowRect(hwnd, &rc);
	...
	{
		Sleep(2);
		PostMessage(hwnd, WM_MBUTTONDOWN, 1, (int)(((int)(ScalingY * 508) << 16) | ((int)(ScalingX * (960)) & 0xFFFF)));
		PostMessage(hwnd, WM_MBUTTONUP, 1, 0);
		Sleep(2);
	}
	else
	{
		Sleep(2);
		PostMessage(hwnd, WM_MBUTTONDOWN, 1, (int)(((int)(ScalingY * 507) << 16) | ((int)(ScalingX * (960)) & 0xFFFF)));
		PostMessage(hwnd, WM_MBUTTONUP, 1, 0);
		Sleep(2);
	}
	MouseMoveState = !MouseMoveState;
